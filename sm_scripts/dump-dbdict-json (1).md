...
1. **Работает ли `new XML(xmlText)` в вашем движке** (E4X). Если нет — скажите, перепишу без E4X, на regex-парсере.
